...
Ltac smash_sm1  := let tac := fun _ => (eauto 1 with sm) in smash_sm_tac tac.
Ltac smash_sm2  := let tac := fun _ => (eauto 2 with sm) in smash_sm_tac tac.
Ltac smash_sm3  := let tac := fun _ => (eauto 3 with sm) in smash_sm_tac tac.
Ltac smash_sm4  := let tac := fun _ => (eauto 4 with sm) in smash_sm_tac tac.
Ltac smash_sm5  := let tac := fun _ => (eauto 5 with sm) in smash_sm_tac tac.
Ltac smash_sm6  := let tac := fun _ => (eauto 6 with sm) in smash_sm_tac tac.
Ltac smash_sm7  := let tac := fun _ => (eauto 7  with sm) in smash_sm_tac tac.
Ltac smash_sm8  := let tac := fun _ => (eauto 8  with sm) in smash_sm_tac tac.
Ltac smash_sm9  := let tac := fun _ => (eauto 9  with sm) in smash_sm_tac tac.
Ltac smash_sm10 := let tac := fun _ => (eauto 10 with sm) in smash_sm_tac tac.
...
Ltac smLR :=
  first [complete auto
        |complete (left;  autorewrite with eo sm in *; eauto 2 with eo sm; smLR)
        |complete (right; autorewrite with eo sm in *; eauto 2 with eo sm; smLR)].
         ...
         |idtac;[];
          ...
          first[fail
               |idtac;[];
                ...
                exrepnd;
                repeat (eexists;[]);
                dands; eauto; eauto 3 with eo sm;
...
Ltac smash_sm_ind ind :=
  ...
  let ind_tac  := (fun _ => eauto 2 with sm) in
  smash_sm_ind_tac ind base_tac ind_tac.

Ltac prove_left := eauto 2 with sm; repndors; tcsp; []; left.
...
Ltac smash_sm_ind3 ind :=
  let base_tac := (fun _ => smash_sm) in
  let ind_tac  := (fun _ => eauto 3 with sm) in
  smash_sm_ind_tac ind base_tac ind_tac.

Ltac smash_sm_ind4 ind :=
  let base_tac := (fun _ => smash_sm) in
  let ind_tac  := (fun _ => eauto 4 with sm) in
  smash_sm_ind_tac ind base_tac ind_tac.

Ltac smash_sm_ind5 ind :=
  let base_tac := (fun _ => smash_sm) in
  let ind_tac  := (fun _ => eauto 5 with sm) in
  smash_sm_ind_tac ind base_tac ind_tac.

Ltac smash_sm_ind6 ind :=
  let base_tac := (fun _ => smash_sm) in
  let ind_tac  := (fun _ => eauto 6 with sm) in
  smash_sm_ind_tac ind base_tac ind_tac.
...
Ltac smash_sm_ind6_7 ind :=
  let base_tac := (fun _ => smash_sm6) in
  let ind_tac  := (fun _ => eauto 7 with sm) in
  smash_sm_ind_tac ind base_tac ind_tac.

Ltac smash_sm_ind6_8 ind :=
  let base_tac := (fun _ => smash_sm6) in
  let ind_tac  := (fun _ => eauto 8 with sm) in
  smash_sm_ind_tac ind base_tac ind_tac.

Ltac smash_sm_ind6_9 ind :=
  let base_tac := (fun _ => smash_sm6) in
  let ind_tac  := (fun _ => eauto 9 with sm) in
  smash_sm_ind_tac ind base_tac ind_tac.

Ltac smash_sm_ind6_10 ind :=
  let base_tac := (fun _ => smash_sm6) in
  let ind_tac  := (fun _ => eauto 10 with sm) in
  smash_sm_ind_tac ind base_tac ind_tac.
