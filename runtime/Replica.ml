...
let run_replica_on_input mynfo (m : msg) : directedMsgs =
  (***PRINTING***)
  (if debug then print_endline (kGRN ^ "[input message: " ^ Batteries.String.of_list (msg2string (Obj.magic m)) ^ "]" ^ kNRM) else ());
...
let rec send_outputs mynfo (out : directedMsgs) (nfos : conn_nfo list) : unit Deferred.t =
  ...
  | dmsg :: dmsgs ->
     ...
     let ndelay = q_dt_approx (Obj.magic delay) in
...
let mk_message_if_some_dest (names : name list) (msg : msg) (delay : int) : directedMsgs =
  ...
  | _ -> [{ dmMsg   = Obj.magic msg;
            dmDst   = names;
            dmDelay = Obj.magic (q_dt_nat_inj (Obj.magic delay)) }];;
...
let rec run_replica_on_inputs mynfo nfos (msgs : (msg * int) list) : unit Deferred.t =
  ...
  | (msg,delay) :: msgs ->
       within'
         ...
         (fun () -> Clock.after (Time.Span.of_sec (float_of_int delay /. 1000.))
                    >>= fun () ->
                    (if debug then print_endline ("[running process on input]") else ());
                    ...
                    (***PRINTING***)
                    (if debug then print_endline ("[sending outputs]") else ());
                    (if debug then print_outputs dmsgs else ());
...
let rec read_inputs mynfo init buffer r (nfos : conn_nfo list) : unit Deferred.t =
  ...
  | `Ok bytes_read ->
     (*(if debug then print_endline ("[bytes read (bufer size: " ^ string_of_int bytes_read ^ ") (total size " ^ string_of_int (Marshal.total_size buffer 0) ^ "): " ^ String.sub buffer 0 bytes_read ^ "]") else ());*)
     (if debug then print_endline ("[(bufer size: " ^ string_of_int buffer_size ^ ") (bytes read: " ^ string_of_int bytes_read ^ ") (total size " ^ string_of_int (Marshal.total_size buffer 0) ^ ")]") else ());
