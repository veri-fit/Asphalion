...
Section PBFTlearns_or_knows_nv.
  ...
  Definition pbft_nv_knows_i (i : pbft_nv_info) (s : PBFTstate) : Prop :=
    pbft_nv_knows i s.

  Lemma pbft_nv_knows_i_if :
    forall d m, pbft_nv_knows d m -> pbft_nv_knows_i d m.
  Proof.
    tcsp.
  Qed.
  ...
  Definition pbft_nv_data2main_auth_data_list (d : pbft_nv_data) : list AuthenticatedData :=
    [new_view2main_auth_data d].
  ...
  Definition pbft_nv_output2data (m : DirectedMsg) : list pbft_nv_data := [].
  ...
  Proof.
    exact (MkLearnAndKnow
             ...
             pbft_nv_data2main_auth_data_list
             pbft_nv_verify
             _ pbft_nv_no_initial_memory).
  Defined.

  Definition knows2
             {eo : EventOrdering}
             (e : Event)
             (d : @lak_data PBFT_I_Data PBFT_I_Node PBFT_I_Key PBFT_I_Msg PBFT_I_Quorum PBFT_I_AuthTok DTimeContextQ PBFT_I_IOTrustedFun PBFT_I_SysOutput 2 PBFT_I_LearnAndKnow_nv) :=
    @knows PBFT_I_Data PBFT_I_Node PBFT_I_Key PBFT_I_Msg PBFT_I_Quorum PBFT_I_AuthTok DTimeContextQ PBFT_I_IOTrustedFun PBFT_I_SysOutput 2 PBFT_I_LearnAndKnow_nv eo e d.

  Definition knew2
             {eo : EventOrdering}
             (e : Event)
             (d : @lak_data PBFT_I_Data PBFT_I_Node PBFT_I_Key PBFT_I_Msg PBFT_I_Quorum PBFT_I_AuthTok DTimeContextQ PBFT_I_IOTrustedFun PBFT_I_SysOutput 2 PBFT_I_LearnAndKnow_nv) :=
    @knew PBFT_I_Data PBFT_I_Node PBFT_I_Key PBFT_I_Msg PBFT_I_Quorum PBFT_I_AuthTok DTimeContextQ PBFT_I_IOTrustedFun PBFT_I_SysOutput 2 PBFT_I_LearnAndKnow_nv eo e d.

  Definition learns2
             {eo : EventOrdering}
             (e : Event)
             (d : @lak_data PBFT_I_Data PBFT_I_Node PBFT_I_Key PBFT_I_Msg PBFT_I_Quorum PBFT_I_AuthTok DTimeContextQ PBFT_I_IOTrustedFun PBFT_I_SysOutput 2 PBFT_I_LearnAndKnow_nv) :=
    @learns PBFT_I_Data PBFT_I_Node PBFT_I_Key PBFT_I_Msg PBFT_I_Quorum PBFT_I_AuthTok PBFT_I_ContainedAuthData DTimeContextQ PBFT_I_IOTrustedFun PBFT_I_SysOutput 2 PBFT_I_LearnAndKnow_nv eo e d.

  Definition learned2
             {eo : EventOrdering}
             (e : Event)
             (d : @lak_data PBFT_I_Data PBFT_I_Node PBFT_I_Key PBFT_I_Msg PBFT_I_Quorum PBFT_I_AuthTok DTimeContextQ PBFT_I_IOTrustedFun PBFT_I_SysOutput 2 PBFT_I_LearnAndKnow_nv) :=
    @learned PBFT_I_Data PBFT_I_Node PBFT_I_Key PBFT_I_Msg PBFT_I_Quorum PBFT_I_AuthTok PBFT_I_ContainedAuthData DTimeContextQ PBFT_I_IOTrustedFun PBFT_I_SysOutput 2 PBFT_I_LearnAndKnow_nv eo e d.

  Definition learns_or_knows2 (eo : EventOrdering) :=
    @learns_or_knows PBFT_I_Data PBFT_I_Node PBFT_I_Key PBFT_I_Msg PBFT_I_Quorum PBFT_I_AuthTok PBFT_I_ContainedAuthData DTimeContextQ PBFT_I_IOTrustedFun PBFT_I_SysOutput 2 PBFT_I_LearnAndKnow_nv eo.
  ...
  Proof.
    ...
    remember (trigger_op e) as trig; destruct trig; auto; eauto 3 with pbft.
  Qed.
  Hint Resolve auth_data_in_trigger_nv_implies_pl : pbft.

  Lemma in_bind_op_list_nv_implies_pl :
    forall (eo : EventOrdering) (e : Event) nv pl pi,
      In (pbft_nv_data2main_auth_data nv) (bind_op_list PBFTget_contained_auth_data (trigger_op e))
      -> In pi (mergeP (new_view2cert nv))
      -> prepare_like_in_prepared_info pl pi
      -> In (pbft_pl_data2main_auth_data pl) (bind_op_list PBFTget_contained_auth_data (trigger_op e)).
  Proof.
    introv ad i prep.
  ...
  Lemma pbft_nv_data2main_auth_data_in_trigger_implies :
    forall (eo : EventOrdering) (e : Event) nv,
      auth_data_in_trigger (pbft_nv_data2main_auth_data nv) e
      -> msg_triggered_event (PBFTnew_view nv) e.
  Proof.
    introv j; unfold msg_triggered_event.
    unfold auth_data_in_trigger in j.

    remember (trigger_op e) as trig; destruct trig; simpl in *; tcsp.
