...
Section PBFT_A_1_9_misc4.
  ...
  Proof.
    ...
    destruct x, a, log_entry_pre_prepare_info, log_entry_request_data; simpl in *;
      ...
      simpl in *; smash_pbft; ginv; simpl in *; smash_pbft.
  ...
  Proof.
    induction pps; introv add prep; repeat (simpl in *; tcsp; smash_pbft);
      dup prep as prep';
      eapply IHpps in prep'; clear IHpps; try eassumption;
        repndors; repnd; tcsp;
    ...
    - eapply check_send_replies_preserves_prepared_log in check;[|eauto]; simpl in *.
      eapply add_new_pre_prepare_and_prepare2log_preserves_prepared_log_backward in add;[|eauto].
      repndors; tcsp.
      right.
    ...
    - exrepnd.
      ...
      apply check_send_replies_preserves_low_water_mark in check; autorewrite with pbft in *.
      rewrite <- check; auto.
