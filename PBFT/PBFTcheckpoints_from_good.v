...
(*    \/ checkpoint_in_view_change cp (view_change_state s)*)
  ...
  Proof.
    ...
    eapply IHP in adds; eauto;[]; clear IHP.
    apply check_send_replies_preserves_checkpoint_certificate_of_last_stable_checkpoint in check.
    rewrite check in adds.
    ...
    forall (eo : EventOrdering) (e : Event) cp i st,
      ...
      -> exists e' st',
          ...
          (
            ...
            (
              msg_triggered_event (PBFTcheckpoint cp) e'
  ...
  Proof.
...

    {
      ...
      - (* the new-view was received *)
        ...
        pose proof (ckeys e e' i s') as eqkeys1; autodimp eqkeys1 hyp; eauto 3 with pbft eo;[].
      ...
      - (* the new-view was sent *)
        ...
        + (* we received the view-change *)
          ...
          pose proof (ckeys e' e'0 i st0) as eqkeys1; repeat (autodimp eqkeys1 hyp);
            try (complete (rewrite eqloc in *; eauto 3 with pbft eo));
            try (complete (simpl in *; congruence));[].
...

    {
      ...
      - (* either we received the checkpoint directly *)
        ...
        assert (loc e' = loc e) as eqloce' by eauto 2 with eo.

        pose proof (ckeys e e' i st') as eqkeys1; repeat (autodimp eqkeys1 hyp);
          eauto 3 with pbft eo; try (complete (simpl in *; congruence));[].
      ...
      - (* or a similar checkpoint was in a view change in our state *)
        ...
        + (* we received the view-change *)
          ...
          pose proof (ckeys e' e'0 i st1) as eqkeys1; repeat (autodimp eqkeys1 hyp);
            try (complete (rewrite eqloc in *; eauto 3 with pbft eo));[].
...

    {
      ...
      - (* either we received the checkpoint message *)
        ...
        pose proof (ckeys e e' i st1) as eqkeys1; repeat (autodimp eqkeys1 hyp);
          eauto 3 with pbft eo; try (simpl in *; congruence);[].
  ...
  Proof.
    ...
    - (* the new-view was received *)
      ...
      pose proof (ckeys e e' i s') as eqkeys1; autodimp eqkeys1 hyp;
        eauto 3 with pbft eo; try (complete (simpl in *; congruence));[].
      applydup localLe_implies_loc in kn0.
    ...
    - (* the new-view was sent *)
      ...
      + (* we received the view-change *)
        ...
        pose proof (ckeys e' e'0 (new_view2sender nv) st0) as eqkeys1; repeat (autodimp eqkeys1 hyp);
          try (complete (eapply state_sm_on_event_some_implies_node_has_correct_trace_before;[|eauto]; simpl; allrw; auto));
          eauto 2 with eo; try (simpl in *; congruence);[].
