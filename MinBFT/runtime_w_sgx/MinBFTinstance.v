...
Definition minbft_data_message := nat.
...
Definition minbft_create_hash_usig_stub (hd : HashData) (k : minbft_sending_key) : MinBFT_digest := minbft_digest_def.
...
Definition minbft_create_hash_usig0
           (hd : HashData)
           (lk : local_key_map) : MinBFT_digest := minbft_digest_def.

Definition minbft_create_hash_usig1
           (hd : HashData)
...
Definition minbft_verify_hash_usig1
           (hd : HashData)
           (d  : MinBFT_digest)
...
Definition minbft_digest2string (d : minbft_digest) : string := d.
...
Definition client2string (c : client C) : string := nat_n2string c.
...
Definition minbft_data_message2string (opr : minbft_data_message) : string := nat2string opr.
...
Definition ui2string (ui : UI) : string :=
  match ui with
  | Build_UI pui d => str_concat ["UI(", pre_ui2string pui, ",", minbft_digest2string d, ")"]
  end.

Definition HashData2string (hd : HashData) : string :=
  match hd with
  | Build_HashData v r pui => str_concat ["HASH-DATA(" , view2string v , "," , request2string r, "," , pre_ui2string pui , ")"]
  end.
...
Definition bare_msg2string (m : MinBFT_Bare_Msg) : string :=
  match m with
  | MinBFT_msg_bare_request r => str_concat ["BARE-REQUEST(" , bare_request2string r , ")"]
  | MinBFT_msg_bare_reply   r => str_concat ["BARE-REPLY("   , bare_reply2string   r , ")"]
  | MinBFT_msg_bare_prepare p pui => str_concat ["BARE-PREPARE(" , bare_prepare2string p , "," , pre_ui2string pui , ")"]
  | MinBFT_msg_bare_commit  c pui => str_concat ["BARE-PREPARE(" , bare_commit2string  c , "," , pre_ui2string pui , ")"]
  end.
...
(* == crypto stuff == *)
Extract Inlined Constant minbft_digest => "string".
Extract Inlined Constant minbft_digest_deq => "(=)".
Extract Inlined Constant minbft_digest_def => "(String.make 1 'x')".
Extract Inlined Constant minbft_digest2string => "Prelude.to_list".
...
Extract Inlined Constant register => "(fun l -> Callback.register (Prelude.of_list l))".
