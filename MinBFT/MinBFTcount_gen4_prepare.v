...
Section MinBFTcount_gen4_prepare.
  ...
  Lemma M_run_ls_on_this_one_event_minbft_preserves_cond_LOGname_prepare :
    forall p {eo : EventOrdering} (e : Event) R s s' subs subs',
      ~ In MAINname (get_names subs)
      -> lower_head 1 subs = true
      -> wf_procs subs
      -> are_procs_n_procs subs
      -> trigger e = trigger_info_data (MinBFT_prepare p)
  ...
  Proof.
    ...
    Time post_minbft_dest_msg;
      ...
      repeat (try gdest; smash_minbft1_at_ eqls0; repeat hide_break; repnd; simpl in *;
              ...
              simpl in *; repndors; ginv; tcsp; eauto 2 with minbft; minbft_simp);
