...
Section MinBFTcount_gen5_commit.
  ...
  Lemma accepted_counter_if_received_UI_primary_commit :
    forall R s x subs s' subs' out r i l,
      ...
      -> M_run_ls_on_input (MinBFTlocalSys_newP R s subs) (msg_comp_name 0) (MinBFT_commit x)
  ...
  Proof.
    ...
    Time post_minbft_dest_msg;
      ...
      try (complete (apply in_subs_find_name_none_implies in iL; tcsp));
      try (complete (unfold lower_out_break in *; simpl in *;
                     ...
                     eexists;
                     first [exists (commit2request_data_i x)
